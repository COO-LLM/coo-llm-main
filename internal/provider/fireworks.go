...
func NewFireworksProvider(cfg *LLMConfig) *FireworksProvider {
	config := openai.DefaultConfig(cfg.APIKey())
	config.BaseURL = "https://api.fireworks.ai/inference/v1"
	client := openai.NewClientWithConfig(config)
	return &FireworksProvider{cfg: cfg, client: client}
}

func (p *FireworksProvider) Name() string {
	return string(ProviderFireworks)
}

func (p *FireworksProvider) Generate(ctx context.Context, req *LLMRequest) (*LLMResponse, error) {
	...
	for attempt := 0; attempt < maxRetries; attempt++ {
		...
		if attempt == maxRetries-1 {
			// Last attempt failed
			if err != nil {
				return nil, fmt.Errorf("Fireworks API error after %d attempts: %w", maxRetries, err)
			}
			return nil, fmt.Errorf("no response from Fireworks after %d attempts", maxRetries)
		}
	}
...
func (p *FireworksProvider) GenerateStream(ctx context.Context, req *LLMRequest) (<-chan *LLMStreamResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("fireworks stream API error: %w", err)
	}
...
func (p *FireworksProvider) CreateEmbeddings(ctx context.Context, req *EmbeddingsRequest) (*EmbeddingsResponse, error) {
	// Fireworks supports embeddings through OpenAI-compatible API
	// Retry with different keys if fail (max 3 attempts)
	maxRetries := 3
	for attempt := 0; attempt < maxRetries; attempt++ {
		// Select least loaded key for first attempt, then rotate on retry
		...
		if attempt == maxRetries-1 {
			// Last attempt failed
			if err != nil {
				return nil, fmt.Errorf("Fireworks embeddings API error after %d attempts: %w", maxRetries, err)
			}
			return nil, fmt.Errorf("no embeddings response from Fireworks after %d attempts", maxRetries)
		}
	}
...
func (p *FireworksProvider) ListModels(ctx context.Context) ([]string, error) {
	// Fireworks supports various open-source models
	return []string{
